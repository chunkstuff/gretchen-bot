...
class ConfessionData:
	...
	def load(self):
		...
		try:
			...
			logger.info(f"Loaded data: {len(self.submission_queue)} submissions, {len(self.post_queue)} scheduled posts")
		except (FileNotFoundError, json.JSONDecodeError) as e:
			logger.warning(f"Persistence file not found or corrupted: {e}. Starting fresh.")
	...
	def archive(self, confession: Dict):
		...
		try:
			with open(self.archive_file, "a") as f:
				...
				f.write("\n")
			logger.info(f"Archived confession #{confession['id']}")
		except Exception as e:
			logger.error(f"Failed to archive confession #{confession['id']}: {e}")
	...
	def get_scheduled_dates(self):
		...
		for confession in self.post_queue:
			if confession["scheduled_time"]:
				...
				# Convert to UK timezone if it's timezone-aware, or assume UTC if naive
